...
def mass_messages_job(job_name, other_args, use_logging=True):
    """
    ...
    with get_cursor() as cur:
        ...
        try:
            ...
            resp = evo_request_with_retries(
                "message/sendText",
                {
                    "number": _phone_number(other_args["p"]),
                    "text": other_args["message"],
            ...
            # mark_message_failure_in_sql(cur, other_args["p"], "failed")
            # log(exception_to_json(e))
            mark_message_failure_in_sql(cur, other_args["p"], exception_to_json(e))
            raise  # Re-raise so the scheduler knows this job failed

        # If message was sent successfully, mark it as success in DB
        mark_message_success_in_sql(cur, other_args["p"])
...
def schedule_mass_messages_jobs(scheduler, cur, numbers, message):
    ...
    for p, run_time in zip(numbers, run_times):
        ...
        CreateJob(
            ...
            other_args={"message": message, "p": p},
            run_time=run_time,
            description="",  # optional description
            batch_id="send_mass_messages_batch",  # group all jobs under one batch
            misfire_grace_time=1  # if the scheduler misses the exact time, only allow 1s grace
        )
